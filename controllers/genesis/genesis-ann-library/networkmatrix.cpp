...
void ESNetwork::setInput( const float *Input, unsigned int size ) { // sets the input of the input neurons, called by trainOutputs() or manually from main.cpp
    ...
    switch( withRL ) { // Set to td_error for actor-critic learning and otherwise to default
        ...
        case 2:// RLS and LMS 
            ...
            for ( unsigned int i = 0; i < outputNeurons; i++ ) {
                ...
                if ( outnonlinearity == 2 ) {// output has tanh transfer function
...
void ESNetwork::takeStep( const float * targets, float learningRate, float td_error , bool learn, int timestep, double param ) {
    ...
    if ( ! leak ) { // no leak, means lealk=0, no memory

        temp->mult( *startweights, *inputs );

        if( feedbackConnections ) {// wheather back connection
