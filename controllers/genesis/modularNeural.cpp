...
void ModularNeural::step(unsigned int ID,CPGSTYPE CPGSType) {
    ...
    mCPGs.at(ID)->setInput(grf.at(ID), np_grf.at(ID), ori, phaseModulationGain /*mAFGs.at(ID)->getOutput()*/,  phaseResetThreshold,  getmCPGsOutputs(n_CPGs),  CPGSType, cmatrix); //force feedback to this
